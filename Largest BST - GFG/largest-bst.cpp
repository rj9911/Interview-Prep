...
class Solution{
    ...
    // { isBST , sizeofBST, minelem , maxelem }
    vector<int> solve(Node* root){
        if(!root) return {1,0, INT_MAX, INT_MIN}; //Base Case
        if(!root->left and !root->right) return {1, 1, root->data, root->data}; // When only either of one node exists
        // Leaf Node
        ...
        return { 0 , max(l[1], r[1]), 0, 0 }; // If tree is not BST.
    }
