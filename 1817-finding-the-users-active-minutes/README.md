<h2><a href="https://leetcode.com/problems/finding-the-users-active-minutes/">1817. Finding the Users Active Minutes</a></h2><h3>Medium</h3><hr><div bis_skin_checked="1"><p>You are given the logs for users' actions on LeetCode, and an integer <code>k</code>. The logs are represented by a 2D integer array <code>logs</code> where each <code>logs[i] = [ID<sub>i</sub>, time<sub>i</sub>]</code> indicates that the user with <code>ID<sub>i</sub></code> performed an action at the minute <code>time<sub>i</sub></code>.</p>
